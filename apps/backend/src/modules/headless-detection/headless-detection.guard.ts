...
export class HeadlessDetectionGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    try {
      // ヘッドレスブラウザ検出を実行
      const result = await this.headlessDetectionService.analyze(request)
      ...
      switch (result.verdict) {
        case 'definite_bot':
          ...
          if (
            config.definiteBotAction === HeadlessDetectionAction.DEGRADE_QUALITY
          ) {
        ...
        case 'likely_bot':
          ...
          if (
            config.likelyBotAction === HeadlessDetectionAction.DEGRADE_QUALITY
          ) {
            this.logger.log(
