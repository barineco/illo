...
export class ReportsService {
  ...
  async createReport(reporterId: string, dto: CreateReportDto) {
    ...
    if (dto.type === ReportType.ARTWORK) {
      if (!dto.artworkId) {
        throw new BadRequestException(
          'artworkId is required for artwork reports',
        )
      }
      ...
      if (!dto.commentId) {
        throw new BadRequestException(
          'commentId is required for comment reports',
        )
      }
    ...
    this.logger.log(
      `Report created: ${report.id} by user ${reporterId}, type: ${dto.type}`,
    )
    return report
  }
  ...
  async updateReport(reportId: string, adminId: string, dto: UpdateReportDto) {
    ...
    this.logger.log(
      `Report ${reportId} updated by admin ${adminId}: status=${dto.status}`,
    )
  ...
  async getReportStats() {
    const [pending, investigating, resolved, dismissed, total] =
      await Promise.all([
