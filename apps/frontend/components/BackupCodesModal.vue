    ...
    >
      ...
      >
        ...
        <div class="space-y-4">
          ...
          <div
            v-else-if="showFullCodes && fullBackupCodes.length > 0"
            class="space-y-4"
          >
          ...
          <div v-else-if="maskedCodes.length > 0" class="space-y-4">
            ...
            >
              <p class="text-sm text-[var(--color-text-muted)] mb-3">
                {{ $t('twoFactor.available') }}: {{ remainingCount }} /
                {{ totalCount }}
              </p>
              <div class="grid grid-cols-2 gap-3">
                <div
            ...
            <p
              class="text-xs text-[var(--color-text-muted)] text-center flex items-center justify-center gap-1"
            >
              <Icon name="ExclamationTriangle" class="w-4 h-4" />
...
const maskedCodes = ref<
  Array<{ masked: string; isUsed: boolean; usedAt: Date | null }>
>([])
...
const fetchBackupCodes = async () => {
  ...
  try {
    const response = await api.get<BackupCodesResponse>(
    ...
    error.value =
      err.response?.data?.message || t('twoFactor.fetchBackupCodesFailed')
  } finally {
    loading.value = false
  }
}
