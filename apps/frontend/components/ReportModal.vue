    ...
    >
      ...
      >
        ...
        <div v-if="submitted" class="p-6 text-center">
          <Icon
            name="CheckCircle"
            class="w-16 h-16 mx-auto text-[var(--color-success-icon)] mb-4"
          />
          <h3 class="text-lg font-medium mb-2">{{ $t('report.submitted') }}</h3>
          <p class="text-[var(--color-text-muted)] text-sm">
            {{ $t('report.submittedDescription') }}
          </p>
          <BaseButton
            variant="primary"
            size="md"
        ...
        >
          ...
          <div class="mb-4">
            <label class="block text-sm font-medium mb-2"
              >{{ $t('report.reason') }}
              <span class="text-[var(--color-danger-text)]">*</span></label
            >
            <div class="space-y-2">
              <label
          ...
          <div
            v-if="error"
            class="mb-4 p-3 bg-[var(--color-danger-bg)] border border-[var(--color-danger-border)] text-[var(--color-danger-text)] rounded-lg text-sm"
          >
            {{ error }}
          </div>
...
const availableReasons = computed(() => {
  const commonReasons = [
    ...
    },
    {
      ...
      label: t('report.reasons.hateSpeech'),
      description: t('report.reasons.hateSpeechDesc'),
    },
    {
  ...
  if (props.type === 'ARTWORK') {
    return [
      {
        ...
        description: t('report.reasons.inappropriateContentDesc'),
      },
      {
        value: 'SEXUAL_CONTENT' as ReportReason,
        label: t('report.reasons.sexualContent'),
        description: t('report.reasons.sexualContentDesc'),
      },
      {
        value: 'VIOLENCE' as ReportReason,
        label: t('report.reasons.violence'),
        description: t('report.reasons.violenceDesc'),
      },
      {
        value: 'COPYRIGHT_VIOLATION' as ReportReason,
        label: t('report.reasons.copyrightViolation'),
        description: t('report.reasons.copyrightViolationDesc'),
      },
      ...commonReasons,
    ]
    ...
    return [
      {
        value: 'ACCOUNT_SPAM' as ReportReason,
        label: t('report.reasons.accountSpam'),
        description: t('report.reasons.accountSpamDesc'),
      },
      {
        value: 'IMPERSONATION' as ReportReason,
        label: t('report.reasons.impersonation'),
        description: t('report.reasons.impersonationDesc'),
      },
      ...commonReasons,
    ]
  } else {
