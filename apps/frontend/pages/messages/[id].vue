<template>
  <div class="max-w-4xl mx-auto px-4 py-8 h-[calc(100vh-180px)] flex flex-col">
    <!-- Header -->
    <div
      class="flex items-center gap-4 pb-4 border-b border-[var(--color-border)] flex-shrink-0"
    >
      <NuxtLink
        to="/messages"
      ...
      >
        <div
          class="w-10 h-10 rounded-full bg-[var(--color-surface-tertiary)] overflow-hidden"
        >
          <img
      ...
      <div class="ml-auto flex items-center gap-2">
        <div
          ...
          :class="{
            'text-green-600 dark:text-green-400':
              encryptionStatus.encryptionSupported,
        ...
        >
          <Icon
            :name="
              encryptionStatus.encryptionSupported ? 'LockClosed' : 'LockOpen'
            "
            class="w-4 h-4"
          />
        </div>
    ...
    <div
      v-if="
        encryptionStatus &&
        !encryptionStatus.encryptionSupported &&
        encryptionStatus.unsupportedDomains.length > 0
      "
      ...
      <div class="flex items-start gap-3">
        ...
        <div class="text-sm">
          ...
          <p class="text-yellow-700 dark:text-yellow-300 mt-1">
            {{
              $t('messages.encryption.notSupportedDescription', {
                domains: encryptionStatus.unsupportedDomains.join(', '),
              })
            }}
          </p>
        </div>
  ...
  try {
    ...
    if (data.messages.length > 0) {
      ...
      if (newMessages.length > 0) {
        ...
        // Mark as read
        await api.patch(
          `/api/messages/conversations/${conversationId.value}/read`,
        )
        fetchUnreadCount()
